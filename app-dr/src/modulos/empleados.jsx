...
const Empleados = () => {
  ...
  const [cargo, setCargo] = useState("");
  const [anios, setAnios] = useState(0);
  const [selectedId, setSelectedId] = useState(null); // Id del empleado seleccionado para actualización
  ...
  const actualizarEmpleado = async () => {
    try {
      await Axios.put(`http://localhost:3001/update/${selectedId}`, {
        nombre,
        edad,
        pais,
        cargo,
    ...
    <div className="container">
     <h2>Gestión de Personal</h2>
       <form>
        ...
        {selectedId ? (
          <button type="button" className="btn btn-primary" onClick={actualizarEmpleado}>
            Actualizar
          </button>
        ) : (
          <button type="button" className="btn btn-primary" onClick={agregarEmpleado}>
        ...
        <tbody>
          {Array.isArray(empleadosList) && empleadosList.map((empleado) => (
            <tr key={empleado.id}>
              ...
              <td>
                <button className="btn btn-danger" onClick={() => eliminarEmpleado(empleado.id)}>Eliminar</button>
                <button className="btn btn-warning ml-2" onClick={() => seleccionarEmpleado(empleado)}>Actualizar</button>
              </td>
            </tr>
          ))}
        </tbody>
